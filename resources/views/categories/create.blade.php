<form id="form-create-category" action="{{ route('categories.store') }}" method="POST"
    ...
    <div class="tw-flex tw-items-center tw-justify-between tw-px-6 tw-py-4 tw-border-b tw-border-gray-100 tw-bg-white">
        <div>
            <h3 class="tw-text-lg tw-font-semibold tw-text-gray-900 tw-tracking-tight">{{ __('category.modal.create_title') }}</h3>
            <p class="tw-text-sm tw-text-gray-500 tw-mt-0.5">{{ __('category.modal.create_subtitle') }}</p>
        </div>
    ...
    <div class="tw-px-6 tw-py-5 tw-bg-white tw-overflow-y-auto tw-flex-1 tw-space-y-5">
        <div>
            ...
            <input type="text" name="name" id="name" required placeholder="{{ __('category.form.name_placeholder') }}"
